...
// mounts.
var HttpdMountPaths = fmt.Sprintf("httpd", "basic/httpd", nil, map[string]interface{}{
...
func setup(t *testing.T) (*criutil.Crictl, func(), error) {
	...
	// Wait for containerd to boot. Note that this will consume some of the
	// output from stderr, which will not appear in the logs via the
	// cleanup. That's okay -- this is actually a nice side effect, since
	// in the happy case we don't see any startup output, only the output
	// that we actually care about for the test.
	if err := testutil.WaitUntilRead(stderr, "Start streaming server", nil, 10*time.Second); err != nil {
